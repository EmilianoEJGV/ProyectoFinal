...
class PDF extends FPDF {
    ...
    function Header() {
        ...
        if ($this->conexion) {
            ...
            $this->Cell(180, 10, utf8_decode("REPORTE DE ENTRENADORES Y NUTRIÓLOGOS "), 0, 1, 'C', 0);
            $this->Ln(7);
            ...
            /*// Calcular el ancho total de las celdas
            $anchoTotalDeLasCeldas = 10 + 20 + 30 + 80 + 10 + 30 + 60;

            // Calcular la posición central de la tabla
            $centroTabla = ($this->GetPageWidth() - $anchoTotalDeLasCeldas) / 2;

            // Establecer la posición X para centrar la tabla
            $this->SetX($centroTabla);*/
            ...
            $this->Cell(90, 10, utf8_decode('Profesión'), 1, 1, 'C', 1);
...
$consulta_reporte_profesional = $conexion->getConnection()->prepare("SELECT primernombre, primerapellido, correo, edad, celular, profesion FROM tbl_profesionales");

if ($consulta_reporte_profesional) {
    $consulta_reporte_profesional->execute();

    while ($datos_reporte = $consulta_reporte_profesional->fetch(PDO::FETCH_ASSOC)) {
        $i = $i + 1;
        ...
        $pdf->Cell(90, 10, utf8_decode($datos_reporte['profesion']), 1, 1, 'C', 0);
    }
} else {
    die("Error: Unable to prepare the statement.");
}

$pdf->Output('ReporteProfesionales.pdf', 'I');
